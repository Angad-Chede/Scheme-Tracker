...
export default function Results({ user, navigate, resultFilter, setResultFilter, onToggleBookmark }) {
    ...
    return (
      <div style={{ minHeight: 'calc(100vh - 64px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div className="empty">
          ...
          <button className="btn btn-primary" style={{ marginTop: '16px' }} onClick={() => navigate('checker')}>Fill Profile</button>
        </div>
      </div>
    );
  }

  const results = SCHEMES.map(s => ({ scheme: s, result: checkEligibility(s, profile) })).sort((a, b) => b.result.score - a.result.score);
  ...
  const filtered = resultFilter === 'eligible' ? eligible : resultFilter === 'partial' ? partial : resultFilter === 'not' ? notElig : results;
  ...
  return (
    <div className="results-wrapper">
      ...
      <div className="g3 results-stats">
        <div className="stat" style={{ borderTop: '3px solid var(--green)' }}><div className="stat-n" style={{ color: 'var(--green)' }}>{eligible.length}</div><div className="stat-l">Fully Eligible</div></div>
        <div className="stat" style={{ borderTop: '3px solid var(--amber)' }}><div className="stat-n" style={{ color: 'var(--amber)' }}>{partial.length}</div><div className="stat-l">Partially Eligible</div></div>
        <div className="stat" style={{ borderTop: '3px solid var(--red)' }}><div className="stat-n" style={{ color: 'var(--red)' }}>{notElig.length}</div><div className="stat-l">Not Eligible</div></div>
      </div>

      <div className="results-filters">
        {[['all', 'All Results'], ['eligible', '✅ Eligible'], ['partial', '⚠️ Partial'], ['not', '❌ Not Eligible']].map(([k, l]) => (
          <button key={k} className={`chip${resultFilter === k ? ' on' : ''}`} onClick={() => setResultFilter(k)}>{l}</button>
        ))}
      </div>

      <div className="results-list">
        {filtered.map(({ scheme, result }) => {
          const bdrColor = result.status === 'eligible' ? 'var(--green)' : result.status === 'partial' ? 'var(--amber)' : 'var(--red)';
          ...
          const statusBadge = result.status === 'eligible'
            ...
            <div key={scheme.id} className="card" style={{ borderLeft: `4px solid ${bdrColor}` }}>
              <div className="result-card">
                <div className="result-card-left">
                  ...
                  <div>{scheme.benefits.slice(0, 3).map(b => <span key={b} className="tag">✓ {b}</span>)}</div>
                </div>
                <div className="result-card-right">
                  <div style={{ textAlign: 'center' }}>
                    <div className="result-score-number" style={{ fontSize: '30px', fontWeight: 800, color: scoreColor }}>{result.score}%</div>
                    <div className="result-score-label">eligibility</div>
                  </div>
                  <div className="result-card-actions">
                    <button className="btn btn-ghost btn-sm" style={{ color: isBm ? 'var(--blue)' : 'var(--text3)' }} onClick={() => onToggleBookmark(scheme.id)}>
                      🔖 {isBm ? 'Saved' : 'Save'}
                    </button>
                    <button className="btn btn-ghost btn-sm" onClick={() => setOpenDetails(d => ({ ...d, [scheme.id]: !d[scheme.id] }))}>
                      {isOpen ? '▲ Less' : '▼ Details'}
                    </button>
                  </div>
                </div>
              ...
              {isOpen && (
                <div className="result-details">
                  ...
                  <div className="result-apply-row">
                    <a href={scheme.link} target="_blank" rel="noopener noreferrer" className="btn btn-primary btn-sm">🔗 Apply Now</a>
                  </div>
                </div>
              )}
            </div>
